((scope) => {
  ...
  const STICKY_POSITION =
      CSS.supports('position', 'sticky') ? 'sticky' :
      CSS.supports('position', '-webkit-sticky') ? '-webkit-sticky' :
      '';
  ...
  class ParallaxContent extends HTMLElement {
    ...
    attributeChangedCallback(name, oldValue, newValue) {
      ...
      if (rate <= 0)
        throw new DOMException('Parallax only supports rates between 0 and 1');
